...
func TestResourceBuilder(t *testing.T) {
	for _, tt := range []string{"default", "all_set", "none_set"} {
		t.Run(tt, func(t *testing.T) {
			...
			databaseAttrVal, ok := res.Attributes().Get("database")
			assert.True(t, ok)
			if ok {
				assert.Equal(t, "database-val", databaseAttrVal.Str())
			}
			mongodbDatabaseNameAttrVal, ok := res.Attributes().Get("mongodb.database.name")
			assert.True(t, ok)
			if ok {
				assert.Equal(t, "mongodb.database.name-val", mongodbDatabaseNameAttrVal.Str())
			}
