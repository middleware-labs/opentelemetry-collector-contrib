...
type metricPostgresqlQuerySlowCount struct {
	data     pmetric.Metric // data buffer for generated metric.
	config   MetricConfig   // metric config provided by user.
	capacity int            // max observed number of data points added to the metric.
}

// init fills postgresql.query.slow_count metric with initial data.
func (m *metricPostgresqlQuerySlowCount) init() {
	m.data.SetName("postgresql.query.slow_count")
	m.data.SetDescription("Queries that are slower than 10 seconds")
	m.data.SetUnit("1")
	m.data.SetEmptyGauge()
}

func (m *metricPostgresqlQuerySlowCount) recordDataPoint(start pcommon.Timestamp, ts pcommon.Timestamp, val int64) {
	if !m.config.Enabled {
		return
	}
	dp := m.data.Gauge().DataPoints().AppendEmpty()
...
// updateCapacity saves max length of data point slices that will be used for the slice capacity.
func (m *metricPostgresqlQuerySlowCount) updateCapacity() {
	if m.data.Gauge().DataPoints().Len() > m.capacity {
		m.capacity = m.data.Gauge().DataPoints().Len()
	}
}
...
// emit appends recorded metric data to a metrics slice and prepares it for recording another set of data points.
func (m *metricPostgresqlQuerySlowCount) emit(metrics pmetric.MetricSlice) {
	if m.config.Enabled && m.data.Gauge().DataPoints().Len() > 0 {
		m.updateCapacity()
		m.data.MoveTo(metrics.AppendEmpty())
		m.init()
...
func newMetricPostgresqlQuerySlowCount(cfg MetricConfig) metricPostgresqlQuerySlowCount {
	m := metricPostgresqlQuerySlowCount{config: cfg}
	if cfg.Enabled {
		m.data = pmetric.NewMetric()
		m.init()
	}
...
type metricPostgresqlServerUptime struct {
	data     pmetric.Metric // data buffer for generated metric.
	config   MetricConfig   // metric config provided by user.
	capacity int            // max observed number of data points added to the metric.
}

// init fills postgresql.server.uptime metric with initial data.
func (m *metricPostgresqlServerUptime) init() {
	m.data.SetName("postgresql.server.uptime")
...
func (m *metricPostgresqlServerUptime) recordDataPoint(start pcommon.Timestamp, ts pcommon.Timestamp, val float64) {
	if !m.config.Enabled {
		return
	}
	dp := m.data.Sum().DataPoints().AppendEmpty()
...
// updateCapacity saves max length of data point slices that will be used for the slice capacity.
func (m *metricPostgresqlServerUptime) updateCapacity() {
	if m.data.Sum().DataPoints().Len() > m.capacity {
		m.capacity = m.data.Sum().DataPoints().Len()
	}
}
...
// emit appends recorded metric data to a metrics slice and prepares it for recording another set of data points.
func (m *metricPostgresqlServerUptime) emit(metrics pmetric.MetricSlice) {
	if m.config.Enabled && m.data.Sum().DataPoints().Len() > 0 {
		m.updateCapacity()
		m.data.MoveTo(metrics.AppendEmpty())
		m.init()
...
func newMetricPostgresqlServerUptime(cfg MetricConfig) metricPostgresqlServerUptime {
	m := metricPostgresqlServerUptime{config: cfg}
	if cfg.Enabled {
		m.data = pmetric.NewMetric()
		m.init()
	}
...
type MetricsBuilder struct {
	...
	metricPostgresqlQuerySlowCount           metricPostgresqlQuerySlowCount
	...
	metricPostgresqlServerUptime             metricPostgresqlServerUptime
...
func NewMetricsBuilder(mbc MetricsBuilderConfig, settings receiver.CreateSettings, options ...metricBuilderOption) *MetricsBuilder {
	mb := &MetricsBuilder{
		...
		metricPostgresqlQuerySlowCount:           newMetricPostgresqlQuerySlowCount(mbc.Metrics.PostgresqlQuerySlowCount),
		...
		metricPostgresqlServerUptime:             newMetricPostgresqlServerUptime(mbc.Metrics.PostgresqlServerUptime),
...
func (mb *MetricsBuilder) EmitForResource(rmo ...ResourceMetricsOption) {
	...
	mb.metricPostgresqlQuerySlowCount.emit(ils.Metrics())
	...
	mb.metricPostgresqlServerUptime.emit(ils.Metrics())
...
// RecordPostgresqlQuerySlowCountDataPoint adds a data point to postgresql.query.slow_count metric.
func (mb *MetricsBuilder) RecordPostgresqlQuerySlowCountDataPoint(ts pcommon.Timestamp, val int64) {
	mb.metricPostgresqlQuerySlowCount.recordDataPoint(mb.startTime, ts, val)
}
...
// RecordPostgresqlServerUptimeDataPoint adds a data point to postgresql.server.uptime metric.
func (mb *MetricsBuilder) RecordPostgresqlServerUptimeDataPoint(ts pcommon.Timestamp, val float64) {
	mb.metricPostgresqlServerUptime.recordDataPoint(mb.startTime, ts, val)
}
