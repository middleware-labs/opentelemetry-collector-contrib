...
func TestMetricsBuilder(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			defaultMetricsCount++
			allMetricsCount++
			mb.RecordPostgresqlIndexBlocksReadDataPoint(ts, 1, AttributeSourceHeapRead)
			...
			for _, mi := range allMetricsList {
				...
				case "postgresql.analyzed":
					...
					assert.Equal(t, "Number of times a table has been manually analyzed.", mi.Description())
					assert.Equal(t, "{analyze}", mi.Unit())
				...
				case "postgresql.autoanalyzed":
					...
					assert.Equal(t, pmetric.MetricTypeSum, mi.Type())
					assert.Equal(t, 1, mi.Sum().DataPoints().Len())
					assert.Equal(t, "Number of times a table has been automatically analyzed.", mi.Description())
					assert.Equal(t, "{autoanalyze}", mi.Unit())
					assert.True(t, mi.Sum().IsMonotonic())
					assert.Equal(t, pmetric.AggregationTemporalityCumulative, mi.Sum().AggregationTemporality())
					dp := mi.Sum().DataPoints().At(0)
					assert.Equal(t, start, dp.StartTimestamp())
				...
				case "postgresql.autovacuumed":
					...
					assert.Equal(t, pmetric.MetricTypeSum, mi.Type())
					assert.Equal(t, 1, mi.Sum().DataPoints().Len())
					assert.Equal(t, "Number of times a table has been automatically vacuumed.", mi.Description())
					assert.Equal(t, "{autovacuum}", mi.Unit())
					assert.True(t, mi.Sum().IsMonotonic())
					assert.Equal(t, pmetric.AggregationTemporalityCumulative, mi.Sum().AggregationTemporality())
					dp := mi.Sum().DataPoints().At(0)
					assert.Equal(t, start, dp.StartTimestamp())
				...
				case "postgresql.blk_read_time":
					...
					assert.Equal(t, pmetric.MetricTypeSum, mi.Type())
					assert.Equal(t, 1, mi.Sum().DataPoints().Len())
					assert.Equal(t, "Time spent reading data file blocks by backends in this database, in milliseconds (if track_io_timing is enabled, otherwise zero)", mi.Description())
					assert.Equal(t, "ms", mi.Unit())
					assert.True(t, mi.Sum().IsMonotonic())
					assert.Equal(t, pmetric.AggregationTemporalityCumulative, mi.Sum().AggregationTemporality())
					dp := mi.Sum().DataPoints().At(0)
				...
				case "postgresql.blk_write_time":
					...
					assert.Equal(t, pmetric.MetricTypeSum, mi.Type())
					assert.Equal(t, 1, mi.Sum().DataPoints().Len())
					assert.Equal(t, "Time spent writing data file blocks by backends in this database, in milliseconds (if track_io_timing is enabled, otherwise zero)", mi.Description())
					assert.Equal(t, "ms", mi.Unit())
					assert.True(t, mi.Sum().IsMonotonic())
					assert.Equal(t, pmetric.AggregationTemporalityCumulative, mi.Sum().AggregationTemporality())
					dp := mi.Sum().DataPoints().At(0)
				...
				case "postgresql.buffer_hit":
					...
					assert.Equal(t, "The number of disk block hits in the buffer cache, thereby avoiding database reads, tagged with database name.", mi.Description())
					...
					assert.Equal(t, pmetric.NumberDataPointValueTypeInt, dp.ValueType())
					assert.Equal(t, int64(1), dp.IntValue())
					dbnameAttrVal, ok := dp.Attributes().Get("dbname")
					assert.True(t, ok)
					assert.Equal(t, "dbname-val", dbnameAttrVal.Str())
				...
				case "postgresql.connection.count":
					...
					assert.Equal(t, pmetric.MetricTypeGauge, mi.Type())
					assert.Equal(t, 1, mi.Gauge().DataPoints().Len())
					assert.Equal(t, "The number of active connections to this database. If DBM is enabled, this metric is tagged with state, app, db and user", mi.Description())
					...
					postgresqlApplicationNameAttrVal, ok := dp.Attributes().Get("postgresql.application_name")
					assert.True(t, ok)
					assert.Equal(t, "postgresql.application_name-val", postgresqlApplicationNameAttrVal.Str())
					userNameAttrVal, ok := dp.Attributes().Get("user.name")
					assert.True(t, ok)
				...
				case "postgresql.index.blocks_read":
					assert.False(t, validatedMetrics["postgresql.index.blocks_read"], "Found a duplicate in the metrics slice: postgresql.index.blocks_read")
					validatedMetrics["postgresql.index.blocks_read"] = true
					assert.Equal(t, pmetric.MetricTypeSum, mi.Type())
					assert.Equal(t, 1, mi.Sum().DataPoints().Len())
					assert.Equal(t, "The number of disk blocks read by this index.", mi.Description())
					assert.Equal(t, "1", mi.Unit())
					assert.True(t, mi.Sum().IsMonotonic())
					assert.Equal(t, pmetric.AggregationTemporalityCumulative, mi.Sum().AggregationTemporality())
					dp := mi.Sum().DataPoints().At(0)
					...
					assert.True(t, ok)
					assert.Equal(t, "heap_read", sourceAttrVal.Str())
				case "postgresql.index.rows_read":
					assert.False(t, validatedMetrics["postgresql.index.rows_read"], "Found a duplicate in the metrics slice: postgresql.index.rows_read")
					validatedMetrics["postgresql.index.rows_read"] = true
					assert.Equal(t, pmetric.MetricTypeSum, mi.Type())
					assert.Equal(t, 1, mi.Sum().DataPoints().Len())
					assert.Equal(t, "The number of index entries returned by scans on this index.", mi.Description())
				...
				case "postgresql.index_bloat":
					...
					assert.Equal(t, pmetric.MetricTypeGauge, mi.Type())
					assert.Equal(t, 1, mi.Gauge().DataPoints().Len())
					assert.Equal(t, "Estimated index bloat ratio (actual pages / expected pages).", mi.Description())
					assert.Equal(t, "1", mi.Unit())
					dp := mi.Gauge().DataPoints().At(0)
					assert.Equal(t, start, dp.StartTimestamp())
					assert.Equal(t, ts, dp.Timestamp())
				...
				case "postgresql.live_rows":
					...
					assert.Equal(t, pmetric.MetricTypeGauge, mi.Type())
					assert.Equal(t, 1, mi.Gauge().DataPoints().Len())
					assert.Equal(t, "The approximate number of live rows, tagged with relation name.", mi.Description())
					...
					assert.Equal(t, pmetric.NumberDataPointValueTypeInt, dp.ValueType())
					assert.Equal(t, int64(1), dp.IntValue())
					relationNameAttrVal, ok := dp.Attributes().Get("relation_name")
					assert.True(t, ok)
					assert.Equal(t, "relation_name-val", relationNameAttrVal.Str())
				...
				case "postgresql.query.count":
					...
					assert.Equal(t, pmetric.MetricTypeSum, mi.Type())
					assert.Equal(t, 1, mi.Sum().DataPoints().Len())
					assert.Equal(t, "Number of times the statement was executed.", mi.Description())
					assert.Equal(t, "1", mi.Unit())
					assert.False(t, mi.Sum().IsMonotonic())
					assert.Equal(t, pmetric.AggregationTemporalityCumulative, mi.Sum().AggregationTemporality())
					dp := mi.Sum().DataPoints().At(0)
					...
					assert.Equal(t, pmetric.NumberDataPointValueTypeInt, dp.ValueType())
					assert.Equal(t, int64(1), dp.IntValue())
					queryTextAttrVal, ok := dp.Attributes().Get("query_text")
					assert.True(t, ok)
					assert.Equal(t, "query_text-val", queryTextAttrVal.Str())
				...
				case "postgresql.query.total_exec_time":
					...
					assert.Equal(t, pmetric.MetricTypeSum, mi.Type())
					assert.Equal(t, 1, mi.Sum().DataPoints().Len())
					assert.Equal(t, "Total wait time of the normalised timed events in nanaoseconds.", mi.Description())
				...
				case "postgresql.rows_deleted":
					...
					assert.Equal(t, pmetric.MetricTypeGauge, mi.Type())
					assert.Equal(t, 1, mi.Gauge().DataPoints().Len())
					assert.Equal(t, "Rows deleted by queries in this db, tagged with relation name.", mi.Description())
					assert.Equal(t, "{row}/s", mi.Unit())
					dp := mi.Gauge().DataPoints().At(0)
					assert.Equal(t, start, dp.StartTimestamp())
					assert.Equal(t, ts, dp.Timestamp())
					assert.Equal(t, pmetric.NumberDataPointValueTypeInt, dp.ValueType())
				...
				case "postgresql.rows_fetched":
					...
					assert.Equal(t, pmetric.MetricTypeGauge, mi.Type())
					assert.Equal(t, 1, mi.Gauge().DataPoints().Len())
					assert.Equal(t, "Rows fetched by queries in this db, tagged with relation name.", mi.Description())
					assert.Equal(t, "{row}/s", mi.Unit())
					dp := mi.Gauge().DataPoints().At(0)
					assert.Equal(t, start, dp.StartTimestamp())
					assert.Equal(t, ts, dp.Timestamp())
				...
				case "postgresql.rows_inserted":
					...
					assert.Equal(t, pmetric.MetricTypeGauge, mi.Type())
					assert.Equal(t, 1, mi.Gauge().DataPoints().Len())
					assert.Equal(t, "Rows inserted by queries in the db, tagged with relation name.", mi.Description())
					assert.Equal(t, "{row}/s", mi.Unit())
					dp := mi.Gauge().DataPoints().At(0)
					assert.Equal(t, start, dp.StartTimestamp())
					assert.Equal(t, ts, dp.Timestamp())
				...
				case "postgresql.rows_updated":
					...
					assert.Equal(t, pmetric.MetricTypeGauge, mi.Type())
					assert.Equal(t, 1, mi.Gauge().DataPoints().Len())
					assert.Equal(t, "Rows updated by queries in the db, tagged with relation name.", mi.Description())
					assert.Equal(t, "{row}/s", mi.Unit())
					dp := mi.Gauge().DataPoints().At(0)
					assert.Equal(t, start, dp.StartTimestamp())
					assert.Equal(t, ts, dp.Timestamp())
				...
				case "postgresql.table_bloat":
					...
					assert.Equal(t, pmetric.MetricTypeGauge, mi.Type())
					assert.Equal(t, 1, mi.Gauge().DataPoints().Len())
					assert.Equal(t, "Estimated table bloat ratio (actual pages / expected pages).", mi.Description())
					assert.Equal(t, "1", mi.Unit())
					dp := mi.Gauge().DataPoints().At(0)
					assert.Equal(t, start, dp.StartTimestamp())
					assert.Equal(t, ts, dp.Timestamp())
				...
				case "postgresql.temp_files":
					...
					assert.Equal(t, pmetric.NumberDataPointValueTypeInt, dp.ValueType())
					assert.Equal(t, int64(1), dp.IntValue())
				case "postgresql.toast.blocks_hit":
					assert.False(t, validatedMetrics["postgresql.toast.blocks_hit"], "Found a duplicate in the metrics slice: postgresql.toast.blocks_hit")
					validatedMetrics["postgresql.toast.blocks_hit"] = true
					assert.Equal(t, pmetric.MetricTypeSum, mi.Type())
					assert.Equal(t, 1, mi.Sum().DataPoints().Len())
					assert.Equal(t, "Number of TOAST block hits.", mi.Description())
					assert.Equal(t, "1", mi.Unit())
					assert.True(t, mi.Sum().IsMonotonic())
					assert.Equal(t, pmetric.AggregationTemporalityCumulative, mi.Sum().AggregationTemporality())
					dp := mi.Sum().DataPoints().At(0)
					...
					assert.Equal(t, pmetric.NumberDataPointValueTypeInt, dp.ValueType())
					assert.Equal(t, int64(1), dp.IntValue())
				case "postgresql.toast.index.blocks_read":
					assert.False(t, validatedMetrics["postgresql.toast.index.blocks_read"], "Found a duplicate in the metrics slice: postgresql.toast.index.blocks_read")
					validatedMetrics["postgresql.toast.index.blocks_read"] = true
					assert.Equal(t, pmetric.MetricTypeSum, mi.Type())
					assert.Equal(t, 1, mi.Sum().DataPoints().Len())
					assert.Equal(t, "Number of TOAST index block reads.", mi.Description())
					assert.Equal(t, "1", mi.Unit())
					assert.True(t, mi.Sum().IsMonotonic())
					assert.Equal(t, pmetric.AggregationTemporalityCumulative, mi.Sum().AggregationTemporality())
					dp := mi.Sum().DataPoints().At(0)
					...
					assert.Equal(t, pmetric.NumberDataPointValueTypeInt, dp.ValueType())
					assert.Equal(t, int64(1), dp.IntValue())
				case "postgresql.toast.size":
					assert.False(t, validatedMetrics["postgresql.toast.size"], "Found a duplicate in the metrics slice: postgresql.toast.size")
					validatedMetrics["postgresql.toast.size"] = true
					assert.Equal(t, pmetric.MetricTypeGauge, mi.Type())
					assert.Equal(t, 1, mi.Gauge().DataPoints().Len())
					assert.Equal(t, "Size of TOAST table.", mi.Description())
					assert.Equal(t, "By", mi.Unit())
					dp := mi.Gauge().DataPoints().At(0)
					assert.Equal(t, start, dp.StartTimestamp())
					assert.Equal(t, ts, dp.Timestamp())
				...
				case "postgresql.transactions.duration.max":
					...
					assert.Equal(t, pmetric.MetricTypeGauge, mi.Type())
					assert.Equal(t, 1, mi.Gauge().DataPoints().Len())
					assert.Equal(t, "Max duration of active transactions.", mi.Description())
					assert.Equal(t, "ms", mi.Unit())
					dp := mi.Gauge().DataPoints().At(0)
					assert.Equal(t, start, dp.StartTimestamp())
					assert.Equal(t, ts, dp.Timestamp())
				...
				case "postgresql.transactions.duration.sum":
					...
					assert.Equal(t, pmetric.MetricTypeGauge, mi.Type())
					assert.Equal(t, 1, mi.Gauge().DataPoints().Len())
					assert.Equal(t, "Sum of duration of active transactions.", mi.Description())
					assert.Equal(t, "ms", mi.Unit())
					dp := mi.Gauge().DataPoints().At(0)
					assert.Equal(t, start, dp.StartTimestamp())
					assert.Equal(t, ts, dp.Timestamp())
				...
				case "postgresql.wal.age":
					...
					assert.Equal(t, pmetric.NumberDataPointValueTypeInt, dp.ValueType())
					assert.Equal(t, int64(1), dp.IntValue())
				case "postgresql.wal.count":
					assert.False(t, validatedMetrics["postgresql.wal.count"], "Found a duplicate in the metrics slice: postgresql.wal.count")
					validatedMetrics["postgresql.wal.count"] = true
					assert.Equal(t, pmetric.MetricTypeGauge, mi.Type())
					assert.Equal(t, 1, mi.Gauge().DataPoints().Len())
					assert.Equal(t, "Number of WAL files.", mi.Description())
					assert.Equal(t, "1", mi.Unit())
					dp := mi.Gauge().DataPoints().At(0)
					assert.Equal(t, start, dp.StartTimestamp())
					assert.Equal(t, ts, dp.Timestamp())
				...
				case "postgresql.wal.lag":
					...
					assert.True(t, ok)
					assert.Equal(t, "replication_client-val", replicationClientAttrVal.Str())
				case "postgresql.wal.size":
					assert.False(t, validatedMetrics["postgresql.wal.size"], "Found a duplicate in the metrics slice: postgresql.wal.size")
					validatedMetrics["postgresql.wal.size"] = true
					assert.Equal(t, pmetric.MetricTypeGauge, mi.Type())
					assert.Equal(t, 1, mi.Gauge().DataPoints().Len())
					assert.Equal(t, "Total size of WAL files.", mi.Description())
					assert.Equal(t, "By", mi.Unit())
					dp := mi.Gauge().DataPoints().At(0)
					assert.Equal(t, start, dp.StartTimestamp())
					assert.Equal(t, ts, dp.Timestamp())
